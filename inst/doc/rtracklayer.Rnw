...
The base track data structure is \emph{RangedData}, as defined by the
\textbf{IRanges} package. \textbf{rtracklayer} supports the import
and export of tracks from and to files in various formats, see
Section~\ref{sec:import-export}. All positions in a \emph{RangedData}
...
\subsubsection{Constructing the \textit{RangedData}}
...
dataset. First, we construct an \textit{IRanges} instance for holding
the start, end and name of each target site. This is then passed to
the \texttt{GenomicData} function,
which constructs a \textit{RangedData} instance. The stand
information, the name of the microRNA and the Ensembl ID of the
targeted transcript are stored as columns in the
\textit{RangedData}. The chromosome for each site is passed as the
\texttt{chrom} argument.
% 
<<rtl-miRNA-track>>=
head(targets)
library(IRanges)
...
\textit{RangedData} object. It holds the feature start and end
positions along with any number of data columns. The ranges and
data are grouped by chromosome (or other sequence).

The primary feature attributes are the \texttt{start}, \texttt{end},
\texttt{chrom} and \texttt{strand}. There are accessors for each
...
@
%
The \texttt{chrom} and \texttt{strand} accessors are defined by the
\textbf{rtracklayer} package, for use with genomic ranges. They are
simple wrappers around \texttt{names(targetTrack)} and
\texttt{targetTrack\$strand}, respectively. The strand is just
another variable in the data and should be a factor with levels
\textit{+}, \textit{-} and \textit{*} (either).
...
\subsubsection{Subsetting a \textit{RangedData}}

It is often helpful to extract subsets from \textit{RangedData}
...
The features may also be indexed by a \textit{IRanges} instance
(explained later) or by chromosome. We give an example of the latter
below for obtaining all target sites on chromosome 1. The name or
index of a chromosome is passed as the sole argument to the \texttt{[}
function, as in list subsetting.
%
<<subset-chrid>>=
...
Import and export of \textit{RangedData} instances is supported in the
...
the web browser.  The default view of the track attempts to show the
entire track. The view region is determined by a call to
\texttt{range} on the track object.
<<genomeSegment-track>>=
range(chr1Targets)
@ 
The returned value from \texttt{range} is an instance of the
\textit{RangesList} class, which specifies a segment of a genome by
its genome ID, chromosome, and start and end positions.

\paragraph{Exercises}

\begin{enumerate}
\item Get the \textit{RangesList} describing the first feature in
  \texttt{targetTrack}.
...
and pass the \textit{RangesList} of the subtrack, zoomed out by a
...
region <- resize(range(subTargetTrack)[1], 1000, symmetric=TRUE)
...
%
<<track-nrsf>>=
nrsfTrack <- GenomicData(nrsfHits, strand="+", chrom="chr1", genome = "hg18")
@ 
%
