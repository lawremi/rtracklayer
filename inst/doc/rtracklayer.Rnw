...
By default, the \textit{layTrack} function creates a \textit{browserView}, an object that represents a view of a particular set of tracks along a particular region of the genome. For \textbf{UCSC}, this roughly corresponds to one tab or window in the web browser. To override the automatic creation of a view, pass \textit{view = FALSE} to the \textit{layTrack} function.
