...
There are two basic track data structures in Bioconductor:
\Rclass{GRanges}, defined by the \Rpackage{GenomicRanges} package, 
and \Rclass{RangedData}, a more general data structure from the
\Rpackage{IRanges} package. For most purposes, \Rclass{GRanges} is
preferred in the context of \Rpackage{rtracklayer}; however,
\Rclass{RangedData} has some particular use cases. 
...
positions in a \Rclass{RangedData} or \Rclass{GRanges} should be
1-based, as in R itself.
...
Import and export of \Rclass{GRanges} and \Rclass{RangedData}
instances is supported in the following formats: Browser Extended
...
@
%
The \Robject{restoredTrack} object is of class \Rclass{RangedData}. To
obtain a \Rclass{GRanges} object directly upon importing, pass
\Rcode{asRangedData = FALSE} to \Rfunction{import}:
<<import-gr, eval = FALSE>>=
restoredTrack <- import("targets.bed", asRangedData = FALSE)
@
%
Eventually, \Rfunction{import} will return \Rclass{GRanges} objects by
default; the extra parameter is necessary for backwards
compatibility. Thus, if a \Rclass{RangedData} is desired, new code
should specify \Rcode{asRangedData = TRUE} in anticipation of the
...
@
%
The returned object is a \Rclass{RangedData}, even if the data was
originally uploaded as a \Rclass{GRanges} or other object. To get a
\Rclass{GRanges} instead, pass \Rcode{asRangedData = FALSE}. By
default, the segment of the track downloaded is the current default
...
The data values for a track are stored in the columns on the
\textit{RangedData} instance. Often, a track contains a single column
of numeric values, conventionally known as the \textit{score}. The
\texttt{score} function retrieves the column named or \textit{score}
or, if one does not exist, the first column in the
\textit{RangedData}, as long as it is numeric. Otherwise,
...
We then convert the hits, stored as a \textit{Views} object (a
particular type of \textit{IRanges} object), to a \textit{RangedData}
instance.
%
<<track-nrsf>>=
...
@ 
%
\Rfunction{GenomicData} is a convenience function that by default
constructs a \Rclass{RangedData} object. A \Rclass{GRanges} may be
obtained by passing \Rcode{asRangedData = FALSE}; however, a
\Rclass{RangedData} may be preferred in this case, as the
\Robject{nrsfHits} object is stored as a \Rclass{Views} object in
\Robject{nrsfTrack} without any loss of informatqion, in particular
the sequence of chr1.
...
\Rclass{rtracklayer} can be used to download annotation tracks from the UCSC
table browser, thus providing a convenient programmatic alternative to 
the web interface available at \url{http://genome.ucsc.edu/cgi-bin/hgTables}.  
...
describes DNaseI hypersensitivity for K562 cells (an immortalized myelogenous
leukemia line) measured at the University of Washington using 'Digital Genome
Footprinting' (see \url{http://www.ncbi.nlm.nih.gov/pubmed?term=19305407}).
Obtain DNaseI regions near the E2F3 TSS, and for all of hg19:
...
tbl.k562.dgf.e2f3 <- getTable(ucscTableQuery (mySession, track='wgEncodeUwDgf', 
                              range=e2f3.grange, table="wgEncodeUwDgfK562Hotspots"))
tbl.k562.dgf.hg19 <- getTable(ucscTableQuery (mySession, track='wgEncodeUwDgf', 
                                              table="wgEncodeUwDgfK562Hotspots"))
@ 
