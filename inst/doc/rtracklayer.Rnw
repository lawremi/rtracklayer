...
explicitly specify a format. This is the first time we actually make
use of the \texttt{rtracklayer} package.
<<load-rtl>>=
library(rtracklayer)
@ 
<<export, eval = FALSE>>=
...
By default, the \texttt{track<-} function creates a
...
\texttt{view = FALSE} to the \texttt{track<-} function.

The default view of the track attempts to show the entire track. The
view region is determined by a call to \texttt{range} on the
track object.
<<genomeSegment-track>>=
range(chr1Targets)
@ 
...
\textit{RangedData} instance, by \texttt{unlist}ing the
\textit{MIndex} into an \textit{IRanges}.
%
<<track-nrsf>>=
