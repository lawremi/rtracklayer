...
static SEXP alloc_ans(int nrow, int ncol, const int *colmap0)
{
...
static void collect_val(SEXP ans, int row_idx, int col_idx, const int *colmap0,
		const char *val, int val_len)
{
	SEXP ans_col;
	SEXPTYPE col_type;
		...
		SEXP ans, int *row_idx, const int *colmap0)
{
	...
	while ((c = line[i++])) {
		...
		if (ans != R_NilValue && colmap0[col_idx] != NA_INTEGER)
			collect_val(ans, *row_idx, col_idx, colmap0,
				    val, val_len);
		col_idx++;
		val = line + i;
		val_len = 0;
...
static const char *parse_GFF_file(SEXP filexp, SEXP feature_types,
		int *ans_nrow, SEXP ans, const int *colmap0, CharAEAE *pragmas)
{
	     ...
	     lineno += EOL_in_buf)
	{
		...
		if (c == '#') {
			...
			if (pragmas != NULL)
				append_string_to_CharAEAE(pragmas, buf);
			continue;
		}
		if (c == '>')
 ...
 *                  columns to collect and in with order.
 ...
 */
SEXP gff_read(SEXP filexp, SEXP colmap, SEXP feature_types)
{
	int colmap0[GFF_NCOL], ans_nrow, ans_ncol;
	const char *errmsg;
	CharAEAE *pragmas;
	SEXP ans, ans_pragmas;

	ans_ncol = prepare_colmap0(colmap0, colmap);
	...
	errmsg = parse_GFF_file(filexp, feature_types,
				&ans_nrow, R_NilValue, colmap0, NULL);
	if (errmsg != NULL)
		error("reading GFF file: %s", errmsg);

	/* 2nd pass */
	pragmas = new_CharAEAE(0, 0);
	PROTECT(ans = alloc_ans(ans_nrow, ans_ncol, colmap0));
	filexp_rewind(filexp);
	errmsg = parse_GFF_file(filexp, feature_types,
				&ans_nrow, ans, colmap0, pragmas);
	if (errmsg != NULL)
		error("reading GFF file: %s", errmsg);

	/* set pragma lines as "pragma" attrib on 'ans' */
	PROTECT(ans_pragmas = new_CHARACTER_from_CharAEAE(pragmas));
	setAttrib(ans, install("pragmas"), ans_pragmas);
	UNPROTECT(2);
	return ans;
}
